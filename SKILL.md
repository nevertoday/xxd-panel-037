---
name: xxd-panel-037
description: "Create XXD Panel 037 artwork from supplied photos in four combinable modes: photo above/source-specific gilded enamel travel magnet or metal badge below, photo left/design right, design alone, or a four-device wallpaper pack with independent or anchor-linked continuity. Use for the exact 037 style; reject cartoon stickers, flat vector icons, plastic 3D, cheap gold borders, thick dramatic shadows, souvenir templates, retail packaging, e-commerce product photography, unrelated landmark decoration."
---

# XXD Panel 037 · 流金珐琅旅行徽章

Compress the photographed subject into one recognisable, collectible travel magnet or badge built from source-derived enamel cells, real metal dividers, a crisp white-metal outer rim, restrained flowing-gold detail, rounded thickness, and a short physical shadow.
...
Use one very short place, subject, theme, or travel-emotion title in the target language. Curve it along the rim, inset it as enamel, cross a negative shape, or make it a metal nameplate; add at most two grounded micro-elements.
...
Use only current attachments, explicit paths, or a previously supplied image clearly identified by the user. Never scan Desktop, `~/Desktop/xxd/xxd-panel-037/`, or historical task folders for a substitute.
...
1. Resolve one or more modes. In a native multi-select use these human-facing labels: `上下对照`, `左右对照`, `只要设计图`, and `四端壁纸`. If no real multi-select exists, ask and wait:
   ...
   1. 上下对照（原图在上，037 设计在下）
   2. 左右对照（原图在左，037 设计在右）
   3. 只要设计图（成品中不显示原图）
   4. 四端壁纸
      手机、iPad、电脑、儿童手表各一张
...
5. Start a fresh job and reserve collision-safe output directories before generation. Use only the current invocation's explicit source or theme. Read `references/xxd-panel-037-prompt.en.md` or `references/xxd-panel-037-prompt.zh-CN.md` immediately before building the generation request.
...
7. Use **single complete-canvas generation as the default for every mode**:
   - `top-bottom`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source in the upper half and the 037 transformation in the lower half.
   - `left-right`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source on the left and the 037 transformation on the right.
   - `design-only`: generate the 037 transformation across the whole canvas; the source is reference-only and not visible.
...
10. Inspect the actual bitmap at full size and thumbnail size. Check, in order: whole-poster integration; 037 aesthetic fidelity; source identity and structure; visual and semantic correspondence between regions; typography and locale; mode, ratio/pixels, count, and PNG format; then approximate 50/50 geometry.
...
COPY PAYLOAD: <the exact 037-specific title and supporting-text package resolved under the local production prompt>
COPY RULE: Render only the populated strings in COPY PAYLOAD, each exactly once. Do not rewrite, translate, spell-correct, duplicate, or add text. Use native shaping, direction, punctuation, spacing, and semantic line breaks. Preserve the 037-specific hierarchy, amount of supporting text, placement, material, and typographic role instead of applying a generic overlay.
```
...
`scripts/compose_panel.py` remains available for deterministic recovery and audit. Trigger it only under step 11. When used, generate a same-aspect design asset from the full 037 prompt, preserve the source without destructive crop or stretch, and document which fallback condition applied. A direct complete-canvas success must not be split and recomposed again.
...
```text
~/Desktop/xxd/xxd-panel-037/<fresh-task>/
└── source-01/
...
Preserve explicit subject, mode set, output count, exact pixels or ratios, wallpaper relationship, copy mode, exact wording, locale, and intended meaning. User overrides may alter these variables but do not silently authorise abandoning source identity, the 037 aesthetic motive, fresh-task isolation, raster-only output, privacy, or verification. If the user explicitly requests a different aesthetic, acknowledge that it leaves this Skill's style rather than pretending it remains 037.

## References

- Read `references/xxd-panel-037-prompt.en.md` or `references/xxd-panel-037-prompt.zh-CN.md` immediately before generation.
- `references/037-source.md` archives the original style brief and is evidence, not an implicit 3:4 default.
